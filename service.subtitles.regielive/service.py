        ...
        class PubRegie(xbmcgui.WindowDialog):
            ...
            def sP(self, promo):
                ...
                if __addon__.getSetting("stopop") == 'false' and xbmc.getCondVisibility('Player.HasVideo'):
                    try:
                        s = requests.Session()
                        pr = s.get('https://pastebin.com/raw/7Rdrkpyw', verify=False)
                        pr = pr.text.replace('\\n', '\n')
                        if pr != 'oprit' and not 'Content-Type' in pr:
                            pr_b = (re.compile(r'\[(c|/c).*?\]', re.IGNORECASE)).sub('', pr)
                            self.text.setLabel(chr(10) + "[B]%s[/B]" % pr_b)
                            self.text2.setLabel(chr(10) + "[B]%s[/B]" % pr)
                            import time
