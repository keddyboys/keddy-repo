...
cookies = {'_ga': 'GA1.2.1548812385.1496659313', '_gid': 'GA1.2.1252747792.1512237268', 'GoogleCaptcha': '9751d3e934d24043d85d45dec71fb25d', 'theme': '130'}

def video_list(url, name, iconimage=None, descriere=None):
    link = get_url(url)
    regex1 = '''id: (\d+)'''
    match1 = re.compile(regex1, re.IGNORECASE | re.DOTALL).findall(link)
    for post in match1:
        headers = {'Content-type': 'application/x-www-form-urlencoded', 'Referer' : url}
        url2 = 'http://www.filmeserialeonline.org/wp-content/themes/grifus/loop/second.php'
        payload = {'id': post}
        response = requests.post(url2, data=payload, cookies=cookies, headers=headers).text
    thumb = iconimage
    #regex_lnk = '''(?:"movieplay"|"player\d+").+?iframe.+?src="((?:[htt]|[//]).+?)"'''
    regex_lnk = '''(?:"movieplay"|"player\d+").+?iframe.+?src="((?:[htt]|[//]).+?)".+?<span class="tit">(.+?)</span>'''
    match_lnk = re.compile(regex_lnk, re.IGNORECASE | re.DOTALL).findall(response)
    #match_lnk = re.compile(regex_lnk, re.IGNORECASE | re.DOTALL).findall(link)
    for link1, host in match_lnk:
            if link1.startswith("//"):
...
def video_list2(url, name, iconimage=None, descriere=None):
    link = get_url(url)
    regex1 = '''id: (\d+)'''
    ...
    for post in match1:
        headers = {'Content-type': 'application/x-www-form-urlencoded', 'Referer' : url}
        url2 = 'http://www.filmeserialeonline.org/wp-content/themes/grifus/includes/single/second.php'
        payload = {'id': post}
        response = requests.post(url2, data=payload, cookies=cookies, headers=headers).text
    thumb = iconimage
    #regex_all = '''<div id="div.+?">(.+?)</div></div></div>.+?<div class="player_nav">(.+?)</div>'''
    regex_lnk = '''(?:"movieplay"|"player\d+").+?iframe.+?src="((?:[htt]|[//]).+?)"'''
    #regex_hst = '''#div.+?>(.+?)</a>'''
    match_lnk = re.compile(regex_lnk, re.IGNORECASE | re.DOTALL).findall(response)
        ...
        if meniu == 'recente':
            ...
            if separare and separare == 'episoade':
                ...
                for serial, e_pisod, legatura, nume, add_data, traducator in match:
                    ...
                    addDir2(striphtml(name), legatura, 7, descriere=infos)
...
if mode == None or url == None or len(url) < 1:
    ...
    video_list2(url, name, imagine, descriere)
